...
tags:
  ...
  - tutorial
canonical: https://gabo.link/minimal-js-carousel
---
...
![anatomy of the carousel](~/assets/images/posts/minimal-js-carousel/anatomy-2.png)
...
First of all, we need to make that carousel content break behind the container or else it'll be contained, all the items, only inside the carousel view box. `overflow-x` to `scroll` makes sure that it'll overflow behind the element when needed, and its scrollable to view the next items.

`scroll-snap` plays a big role on the carousel smooth animation.

# The sprinkle of JS

In summary, javascript will be needed to scroll just enough for the css to smooth the snap animation when an item slide in.
