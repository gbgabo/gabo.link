...
```bash
# comando para setar uma imagem minha como wallpaper de um monitor específico
xwallpaper --output eDP-1 --stretch Pictures/wallpapers/object_mono_dark_purple_horizon_1.png
```

```bash
# comando para habilitar a função de toque para clique no touchpad do notebook
...
- Em **CLI** você possui uma gama de opções disponíveis que podem ser declaradas todas de uma vez, de uma forma mais livre. Como seu prato do _buffet_, que tem muito mais batata frita que deveria.

Isso nem sempre é uma necessidade, até porque ser muito descritivo demanda escrever bem as coisas que o comando precisa. O objetivo não é vocẽ memorizar esses comandos longos. Você pode salvá-los como um `alias`. Alias são como "gírias" que você pode substituir um comando longo. Parte do poder para ser preciso e rápido está nessa ferramenta. Eu posso **navegar para lugares exatos**, **pedir um comando difícil de lembrar**, **Abrir ou criar um arquivo específico** e até **chamar mais de um comando em série**. Existem muitas listas na internet com aliases bom para se ter no sistema. O próprio sistema operacional é formado por muitos alias que chamam outras coisas por trás.
